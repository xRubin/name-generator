...
class Markov {
    ...
    public function makeName($maxLen = 10) {
        ...
        $startExample = mb_str_split($this->NameList[$this->Random->randArray($this->NameList)]);
        $name = $startExample[0] . $startExample[1];

        foreach ($this->MarkovList as $key => $value) {
            $nameChar = mb_str_split($name);
